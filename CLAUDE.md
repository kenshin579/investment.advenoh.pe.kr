...
**Mermaid 특수 문자 처리**: 노드 라벨(`[]`) 안에 특수 문자가 포함되면 렌더링이 깨진다. 다음 문자가 포함된 라벨은 반드시 쌍따옴표(`"`)로 감싼다:

- `%` — Mermaid 주석 기호(`%%`)로 인식됨
- `~` — 제네릭 구문으로 인식됨. `-` 또는 범위 표현으로 대체
- `/`, `(`, `)` — 노드 형태 구문과 충돌 가능
- `<br/>` — 쌍따옴표 밖에서만 줄바꿈으로 동작. 쌍따옴표 안에서는 리터럴 텍스트로 렌더링되므로, 괄호 표기 등으로 대체
