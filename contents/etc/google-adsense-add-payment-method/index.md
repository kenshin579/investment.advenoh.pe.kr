...
## 2.1 결제 추단 추가 클릭 > 새 은행 송금 세부정보 추가 > `저장` 버튼 클릭한다
...
며칠이 지나면 은행에서 연락이 온다. 신한은행에서 당일날 자주 전화가 와서 광고인 줄 알고 무시했었는데, 알고 보니 구글에서 돈이 들어왔으니 처리해드리겠다는 연락이었다. 자동으로 되는 게 아니라 은행에 연락해서 확인하고 처리하는 방식인 듯하다.
