...
연금저축은 **개인이 노후를 대비하여 자발적으로 가입하는 세제혜택 저축 제도**다. 국민연금과 달리 가입이 의무가 아니고, 본인이 원하는 금액을 자유롭게 납입할 수 있다. 정부가 세액공제 혜택을 주는 이유는 간단하다. 국민연금만으로는 부족한 노후 소득을 개인이 스스로 준비하라는 것이다. 핵심 구조는 다음과 같다.
...
연금저축은 가입하는 금융사에 따라 두 가지로 나뉜다. 이름은 같지만 운용 방식과 수익 구조가 완전히 다르다. 자신의 투자 성향에 맞는 상품을 골라야 한다.
...
연금저축과 IRP에 납입하면 세액공제를 받는다. 공제율은 총급여에 따라 달라진다. 소득이 낮을수록 더 높은 공제율(16.5%)이 적용되어 저소득자에게 상대적으로 유리하다. 아래 표에서 본인의 총급여 구간에 해당하는 환급액을 확인해보자.
...
세액공제를 받은 납입금과 운용수익을 연금으로 수령할 때 적용되는 세율이다. 나이가 많을수록 세율이 낮아진다. 고령일수록 소득이 줄어드는 현실을 반영한 것이다. 수령 방식에 따라서도 세율이 다르므로 함께 고려해야 한다.
...
연금 수령 시 가장 중요한 숫자는 **연 1,500만 원**이다. 인출 한도가 아니라 **세금 구조가 달라지는 기준선**이다. 1,500만 원 넘게도 얼마든지 꺼낼 수 있다. 다만 초과하는 순간 세율이 크게 올라가므로, 이 선을 넘기지 않도록 조절하는 것이 핵심이다.
...
25년간 **약 2,970만 원**을 세금에서 돌려받는다. 매년 연말정산 때 약 119만 원이 환급되니 월급 외 보너스처럼 느껴진다. 이 금액만으로도 가입할 이유는 충분하다. 다만 진짜 효과는 과세이연과 인출 전략에서 나온다.

## 5.3 운용 결과: 과세이연 효과

일반 계좌에서는 매년 수익이 발생할 때마다 15.4%의 배당소득세를 내야 한다. 반면 연금계좌는 수령 시점까지 세금이 유예되므로, 세금으로 빠져나갈 돈까지 함께 굴러간다. 25년 동안 이 차이가 복리로 쌓이면 약 6,000만 원까지 벌어진다.
...
적립금 전액을 한 번에 꺼내면, 세액공제 받은 납입금과 운용수익 전체에 **기타소득세 16.5%**가 부과된다. 그동안 받은 세액공제 혜택을 토해내고, 운용수익에까지 높은 세율이 붙는다. 가장 불리한 선택이다.
...
분리과세 한도인 연 1,500만 원 이하로 수령하면, 종합소득세 합산 없이 **저율 연금소득세**만 내면 된다. 나이가 들수록 세율이 낮아지니 오래 나눠 받을수록 총 세금이 줄어든다. 건강보험 피부양자 자격도 그대로 유지된다.
...
세법은 매년 바뀌고, 연금저축 규정도 예외가 아니다. 최근 몇 년은 연금 수령자에게 유리한 방향으로 바뀌었다. 특히 분리과세 한도 상향은 수령 전략에 직접 영향을 주니 반드시 알아두자.
...
지금까지 다룬 내용을 납입과 수령 두 단계로 나누어 체크리스트로 정리한다. 연금저축은 넣을 때 한 번, 꺼낼 때 한 번 전략이 필요하다. 아래 항목을 매년 점검하면 불필요한 세금을 줄일 수 있다.
