...
뉴스에서 "연준이 양적완화를 시작했다", "양적긴축에 돌입했다"는 말을 자주 듣지만, 정확히 무슨 뜻인지 설명하기는 쉽지 않다. 양적완화(QE)와 양적긴축(QT)은 중앙은행이 금리만으로 경기를 조절하기 어려울 때 꺼내 드는 카드이고, 금융시장과 자산 가격을 직접 흔든다.

이 둘은 따로 떨어진 정책이 아니다. **경기 침체 → QE → 회복 → 테이퍼링 → QT**로 이어지는 하나의 사이클이다. 그래서 QE만, QT만 따로 떼서 보면 전체 그림을 놓친다.
...
- 국채 금리는 장기금리의 기준이 되므로, 주택담보대출 금리와 회사채 금리도 함께 내려간다
- 낮아진 금리가 기업의 투자와 가계의 소비를 자극한다
...
미국 연준의 대차대조표는 2008년 약 9,000억 달러에서 2022년 약 9조 달러까지, 10배 넘게 불어났다. QE가 얼마나 큰 규모였는지 이 숫자 하나로 짐작할 수 있다.
...
이 사건을 **테이퍼 탠트럼(Taper Tantrum)**이라 부른다. 실제로 매입을 줄인 것도 아니고 "줄일 수도 있다"는 말 한마디였는데도 시장이 이렇게 출렁였다.
...
일본이 대표적인 사례다. 수십 년간 QE를 지속했지만 디플레이션에서 좀처럼 벗어나지 못했다. 통화정책만으로 풀 수 없는 문제도 있다는 뜻이다.
...
양적완화(QE)와 양적긴축(QT)은 통화정책 사이클을 따라 번갈아 등장한다.
...
2008년 이후 이 사이클은 한층 뚜렷해졌고, 이제 중앙은행의 정책 방향만큼 시장을 크게 좌우하는 변수도 드물다. 투자자라면 연준의 대차대조표 변화를 꾸준히 지켜볼 필요가 있다.
