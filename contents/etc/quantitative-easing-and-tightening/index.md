...
```mermaid
graph LR
    A[중앙은행<br/>국채·MBS 매입] -->|대금 지급| B[시중은행<br/>지급준비금 증가]
    B -->|대출 확대| C[기업·가계<br/>자금 조달 용이]
    C -->|소비·투자 증가| D[실물 경제<br/>경기 회복]
```
...
```mermaid
graph LR
    A[보유 국채<br/>만기 도래] -->|원금 회수| B[중앙은행<br/>대차대조표 축소]
    B -->|지급준비금 감소| C[시중은행<br/>유동성 축소]
    C -->|대출 여력 감소| D[기업·가계<br/>자금 조달 어려움]
```
...
```mermaid
graph TD
    A["1. 경기 침체<br/>실업 증가, 소비 위축"] -->|금리 인하 + QE| B["2. 경기 회복<br/>유동성 확대, 자산 가격 상승"]
    B -->|자산 매입 축소| C["3. 테이퍼링<br/>QE 규모 점진적 축소"]
    C -->|자산 축소 시작| D["4. 양적긴축(QT)<br/>유동성 회수, 금리 상승"]
    D -->|경기 둔화 시| A
```
