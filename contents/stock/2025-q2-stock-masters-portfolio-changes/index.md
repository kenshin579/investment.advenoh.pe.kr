...
투자 대가들은 올해 2분기에 어떤 종목을 사고 팔았을까? 월가 대표 투자자들의 최신 13F 보고서를 바탕으로 주요 운용사와 대가들의 매수/매도 종목과 투자 전략 변화를 정리한다.
...
2025년 2분기 버크셔 해서웨이는 집중 위험을 낮추고 금융 섹터를 재배치했다. 금리 인하를 염두에 둔 포지션 조정으로 보인다.

- `AAPL` 비중이 과도하게 큰 상태에서(여전히 최대임) 추가 축소로 단일 종목 리스크를 낮춤
- `UNH` 신규 편입은 사이버공격, 조사, 경영진 이슈로 밸류에이션이 눌린 우량 기업을 싸게 산 것으로, 전형적인 버핏 스타일이다
...
레이 달리오의 2025년 2분기 포트폴리오는 AI 투자 사이클에 베팅하면서, 중국과 거버넌스 리스크를 줄이고 미국 빅테크로 갈아탄 모습이다. 대신 유틸리티처럼 현금흐름이 안정적인 자산으로 균형을 맞췄다.

- 넓은 베타(`SPY`, `IVV`, `IEMG`) 비중을 살짝 줄이고, 메가테크와 AI 개별주(`NVDA`, `MSFT`, `GOOGL`, `META`)로 무게중심을 옮김
- 중국 주식(알리바바, 핀둬둬, 바이두)을 완전히 청산했다. 과세 이슈에 대비한 것으로 보인다
- 금(`GLD`) 비중도 낮춤
- 유틸리티(GE Vernova)가 상위권에 새로 들어오며 변동성을 완충함
...
2025년 2분기 퍼싱스퀘어는 산업재(철도)를 줄이고 소비재와 플랫폼으로 무게중심을 옮겼다. Q1 상위에 있던 `CP`를 사실상 접고 `AMZN`을 상위권으로 끌어올렸다. 우버, 브룩필드, 레스토랑브랜즈, 칩틀레, 힐튼 등 기존 종목과 함께 소비, 서비스 비중을 두껍게 만들었다. 알파벳(구글) 비중은 다듬어 광고 플랫폼 노출을 유지했고, 종목 수는 최소한으로 묶어 소수 종목 집중 기조를 이어갔다.

- 소비재 비중 확대: `AMZN` 편입, `QSR`, `CMG`, `HLT` 등 기존 소비 테마와의 결합으로 소비재 축이 두터워짐
- 산업재 비중 축소: `CP` 이탈 영향
- 커뮤니케이션/플랫폼 노출 유지, 강화: 구글 비중 조정 지속

경기와 금리에 민감한 자본집약 산업을 줄이고, 네트워크 효과와 가격결정력이 강한 사업으로 위험 대비 기대수익을 높이려는 판단으로 읽힌다.
...
2025년 2분기 켄 피셔는 규모를 키우면서 방향은 더 또렷해졌다. 최대 보유가 Apple에서 Microsoft로 바뀌었고, TSMC와 ASML 같은 반도체 체인, Google과 Meta 같은 플랫폼 노출을 늘려 IT와 커뮤니케이션 축을 강화했다. American Express를 새로 담고 JPM, Morgan Stanley 비중을 늘려 금융 섹터도 키웠다. 반면 Eli Lilly 등 헬스케어는 뒤로 밀렸다. AI/반도체 수요와 플랫폼의 현금창출력에 베팅하면서, 금리와 신용 사이클의 연착륙을 가정해 금융을 늘린 리밸런싱으로 읽힌다.
...
2025년 2분기 드러켄밀러의 포트폴리오는 확신 있는 테마 안에서의 선택과 집중으로 볼 수 있다. 보유 종목 폭은 넓히되, 자본은 바이오 촉매, 반도체 생산능력, 디지털 소비처럼 현금흐름과 성장이 보이는 쪽으로 더 모았다. 종목을 늘려 분산하면서도 핵심 테마에 집중해 단기 변동성을 줄인 리밸런싱이다.

- 바이오 강화: 임상, 규제 촉매에 민감한 `NTRA`, `INSM` 비중을 키워 사건 주도형 수익을 노림
- 반도체 체인 회전: `TSMC` 확대와 `ENTG` 신규는 AI/고급공정 사이클의 파운드리, 소재, 장비에 초점을 맞춘 선택
- 변동성 큰 `COHR` 비중은 다소 줄여 리스크, 리워드 균형을 조정
- 소비, 디지털 레저 재배치: `CPNG` 축소, `FLUT` 편입, MELI 유지로 지역과 수익모델을 분산
...
올해 2분기 대가들은 AI, 반도체, 글로벌 플랫폼, 현금흐름이 탄탄한 소비, 서비스에 힘을 실었다. 반대로 규제나 정책 리스크가 큰 지역과 업종은 줄이거나 더 나은 종목으로 갈아탔다. 잘 오른 건 조금 덜고 확신이 커진 건 더하는 식으로 쏠림을 관리한 것도 공통적이다. 기회가 올 때 바로 쓸 수 있도록 현금을 챙겨두는 모습도 눈에 띈다.
