...
개인이 직접 ETF 종목을 선택하고 분산 투자를 하는 것은 생각보다 쉽지 않다. 각 종목을 분석하고, 매수와 매도 타이밍을 정하고, 여러 자산에 나눠 담아 리스크까지 관리해야 한다. 시간도 공부도 많이 든다. 이런 부분을 신경 쓰지 않고 운용보수를 주고 그냥 사람을 쓰겠다 하면 `TDF`가 좋은 연금용 펀드가 될 수 있다.
...
`TDF`는 목표 날짜에 맞춰 자동으로 자산 배분을 조정해주는 펀드다. 장단점을 알고 자기 은퇴 시점에 맞는 라인업을 고르면 장기 투자에 쓰기 괜찮다.
