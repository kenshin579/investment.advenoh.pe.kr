...
description: "서킷브레이커와 사이드카의 개념과 차이점을 상세히 분석하고, 사이드카(선물-현물 가격괴리 5-6% 1분 지속 시 5분 중단), 서킷브레이커(1단계 8%, 2단계 15%, 3단계 20% 하락 시 20분 거래중단)의 발동 조건과 과거 사례를 정리합니다."
date: 2024-08-10
update: 2024-08-10
category: Stock
tags:
...
`사이드카`는 블랙 먼데이 사태를 계기로 `서킷브레이커`와 함께 도입됐다. 선물 시장의 급격한 등락이 현물 가격에 급작스럽게 번지는 것을 막기 위한 장치다. 오토바이의 `사이드카`처럼 본체(현물 시장)가 흔들리지 않도록 옆에서 잡아준다고 보면 된다. `서킷브레이커`의 전단계에 해당한다.
...
정리하면, `서킷브레이커`는 급락을 멈춰 세우고 투자자에게 대응할 시간을 벌어준다.

> 역대 `서킷브레이커` 발동

과거 `서킷브레이커` 발동 사례를 보면 미국 시장에서 시작된 사건이 대부분이다. 수출 중심인 한국 경제는 미국 증시의 변동성에 곧바로 영향을 받기 때문이다. 그만큼 미국 시장 움직임을 챙겨볼 필요가 있다.
...
`서킷브레이커`와 `사이드카`는 급락 국면에서 잠시 숨을 돌리게 해주는 안전장치다. 이번 주 발동 사례 덕분에 이 둘을 제대로 들여다보게 됐다. 주식 시장은 배울 게 끝이 없어서 재미있으면서도 어렵다.
