...
주식 시장에서 `성장주`와 `가치주`는 서로 다른 투자 접근 방식이다. `성장주`는 미래 성장 가능성을 기반으로 주식을 선택하고, `가치주`는 저평가된 주식을 찾아 투자한다. 각 주식 스타일의 특징을 살펴보자.
...
#### 2.1.2.2 중형 가치: VOE (Vanguard Mid-Cap Value ETF)
...
지금까지 미국의 성장주와 가치주 ETF의 차이, 그리고 각 ETF의 특징을 살펴봤다. 투자 목표와 성향에 따라 대형성장(VUG), 중형성장(IWP), 대형가치(VTV), 중형가치(VOE) 중에서 선택하면 된다. 종목 구성을 직접 확인해 보면 각 ETF가 어떤 성격인지 더 분명하게 와닿을 것이다.
