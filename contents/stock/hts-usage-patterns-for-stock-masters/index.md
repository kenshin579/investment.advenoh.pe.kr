...
시장이 오픈하자마자 바로 매매를 시작하기보다는 먼저 아래 화면들을 보며 지수를 확인하고 현재 시장 상황을 파악한다. 개인·외국인·기관의 매매 동향을 살펴 어느 투자자가 매수 또는 매도를 주도하는지도 체크한다.
...
> **TIP** 주식종합 화면에서 등록한 종목을 선택하면 아래 화면들도 자동으로 변경된다. 덕분에 관련 뉴스, 기업 정보, 차트를 한 번에 확인할 수 있어 매매 여부를 빠르게 판단하는 데 도움이 된다.
...
참고로 기업정보(FnGuide) 화면은 종목에서 우클릭해서도 실행할 수 있다.
...
MTS(모바일 트레이딩 시스템)로도 주식 확인 및 주문이 가능하지만, HTS(홈 트레이딩 시스템)를 사용하면 시간을 훨씬 절약할 수 있다. 전문가만 쓰는 도구가 아니라 누구나 쉽게 활용할 수 있으니 한 번 써보길 권한다.
