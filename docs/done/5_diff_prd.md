...
- **제목**: "원천징수 비율 80%, 100%, 120% — 어떤 걸 선택해야 할까?"
- **시리즈**: "연말정산"
...
- **참고 자료**:
  ...
  - https://incometax.calculate.co.kr/earned-income-tax-calculator (근로소득세 계산기)
...
> - 100% 기준 원천징수 소득세: **195,960원** (+ 지방소득세 19,590원)
> - 80%: **156,760원** (+ 지방소득세 15,670원)
> - 120%: **235,150원** (+ 지방소득세 23,510원)
>
> → 80%와 120%의 월 소득세 차이: **78,390원** (지방소득세 포함 시 약 86,230원)
>
> *(2024년 3월 개정 간이세액표 기준)*

#### 2.3 간이세액표 주요 구간 데이터

> 2024년 3월 개정 간이세액표 기준 (소득세만, 지방소득세 별도)
...
**80%/120% 선택 시 월 원천징수 소득세:**

| 월급여 | 비율 | 부양가족 1인 | 2인 | 3인 |
|:---:|:---:|:---:|:---:|:---:|
| 400만 원 | 80% | 156,760원 | 134,360원 | 87,670원 |
| 400만 원 | 100% | 195,960원 | 167,950원 | 109,590원 |
| 400만 원 | 120% | 235,150원 | 201,540원 | 131,500원 |

- 부양가족 수가 많을수록 원천징수 소득세가 줄어든다
- "전체 공제대상 가족 수"는 본인을 포함한 기본공제대상자 수이다
- 8세 이상 20세 이하 자녀가 있으면 추가로 자녀 1명당 12,500원 공제된다
...
> **총급여 5,000만 원 기준** (월급 약 417만 원, 비과세 제외), 간이세액표 실제 데이터 적용
> - **핵심**: 연간 총 세금(결정세액)은 동일하다. 납부 시점만 다를 뿐이다

#### 4.1 공제 항목이 적은 경우 (미혼, 부양가족 본인 1명, 카드 소비 적음)
...
- 공제가 적으면 80% 선택 시 추가 납부 가능성이 높다
- 다만 추가 납부 금액은 월 1~2만 원 수준을 1년간 미리 쓴 것이므로, 그 자금을 투자에 활용했다면 기회비용으로 상쇄 가능하다

#### 4.2 공제 항목이 많은 경우 (기혼, 부양가족 3명, 연금저축 900만 원 납입)
...
- 공제가 많으면 80%를 선택해도 환급을 받을 수 있다
- 80% 선택 시 매달 약 2.8만 원(100% 대비)을 더 받고, 연말에도 소액 환급
- 120%와 비교하면 매달 약 5.7만 원을 더 받는 대신 연말 환급이 68만 원 줄어든다
...
- [x] **제목 확정**: "원천징수 비율 80%, 100%, 120% — 어떤 걸 선택해야 할까?"
- [x] **시뮬레이션 상세도**: 간이세액표 실제 데이터 기반으로 구체적 금액 포함
- [x] **연말정산 글과의 관계**: 독립 포스팅
- [x] **시리즈 설정**: "연말정산" 시리즈로 묶음
- [x] **간이세액표 데이터**: 2024년 3월 개정 간이세액표 기준 실제 데이터 반영 완료
