...
```mermaid
graph LR
    A[매달 급여 지급] --> B[간이세액표 기준으로<br/>소득세 원천징수]
    B --> C[1년간 원천징수한<br/>세금 합계 = 기납부세액]
    C --> D[연말정산]
    D --> E{기납부세액 vs 결정세액}
    E -->|기납부 > 결정| F[환급 💰]
    E -->|기납부 < 결정| G[추가 납부 😰]
...
1. 회사 인사/급여 담당자에게 **「소득세 원천징수세액 조정신청서」** 제출
2. 또는 홈택스에서 간이세액표를 조회한 후 원하는 비율을 회사에 요청
3. 변경은 **연중 언제든** 가능하다
4. 변경한 달의 다음 급여부터 적용된다

## 5.2 변경 시 고려사항

- **연초에 변경하면** 1년 내내 효과를 볼 수 있다
- 연말에 변경하면 효과가 크지 않다 (이미 대부분 원천징수 완료)
- 매년 연초에 **작년 연말정산 결과를 보고 비율을 조정**하는 것을 추천한다
- 원천징수방식을 변경한 이후에는 재변경 전까지 계속 적용해야 한다 (변경한 과세기간에는 재변경 불가)
