...
>
> 본문에는 검증을 통과한 수치만 실었다. 판정이 `불일치` 또는 `출처확인불가`인 40건은 제외했다. 그래도 투자 판단에 쓰기 전 출처를 직접 확인하기 바란다.
```

- [ ] **Step 3: 본문을 목차대로 작성한다**

본문은 Task 1의 원본 산출물과 Task 2의 검증 문서에서 가져온다. **Task 2 검증 표에서 `불일치`/`출처확인불가`로 판정된 항목과 UNSOURCED 11건, 총 40건은 하나도 싣지 않는다.**

검증 문서 `§10`이 이 판단을 대신해준다. 62행 표를 직접 훑지 말고 아래를 쓴다.

| 절 | 용도 |
|---|---|
| `§10.0` | 그대로 쓸 수 있는 것 (구간별 요약 13개) — `## 2.2`의 재료 |
| `§10.1` | 값과 서술을 모두 버릴 것 (5건) |
| `§10.2` | 값을 교체하면 쓸 수 있는 것 (9건) |
| `§10.3` | 반드시 설명을 붙일 것 (3건) |
| `§10.4` | 확인 못 해 보류 (14건) |
| `§10.5` | 스킬이 `UNSOURCED`로 표기한 11건 |
| `§10.6` | 검증으로 새로 확보한 값 (5건) — `raw.md`에 없던 것이라 써도 된다 |
...
`# 3`에는 Task 2의 집계(62개 중 일치 31 / 근사 2 / 불일치 15 / 출처확인불가 14)와 판정 기준을 넣는다. UNSOURCED 11건을 더해 제외 대상은 40건이고, 그중 5건은 `raw.md`에 값이 없던 누락 항목이라 실제로 걷어낼 값은 35건이다.
...
- `## 4.1` 하이퍼스케일러 capex $600B — 인용된 Sourceability 기사 본문에 해당 수치가 없다. 원문 97,823바이트를 받아 본문 14,351자 전수 검색으로 확인. "Q3 2025 사상 첫 분기 $100B 돌파"도 같은 문장인데 역시 출처에 없다
- `## 4.2` TSMC 파운드리 점유율 67.6% — Q1 2025 기사를 2026년 데이터로 인용. 스킬은 이를 "출처 충돌"로 서술했으나 실제로는 연도 오류. SEMI 총 장비 $139B도 같은 유형(2024-12-08 구판 수치를 2027년 신판 수치와 나란히 배치)
- `## 4.3` WSTS Autumn 2025 전망 $975B — 값은 Sourceability 본문에 실재하나 **Deloitte 귀속**이다. `raw.md`가 WSTS로 잘못 달았다. 값이 맞아도 출처 기관이 다르면 인용이 성립하지 않는다는 사례
- `## 4.4` NXP 47.3%(MEMS 매각 일회성 차익 $627M 포함), QCOM 순이익 > 영업이익(스킬 진단 "영업외 이익"은 오류, 실제는 이연법인세 평가충당금 환입 $5.7B). 다만 NXP·Intel 이상치는 스킬이 정확히 짚었으므로 **원인 진단 3건 중 2건은 맞았다**고 써야 공정하다
