...
```yaml
---
title: "Claude 스킬로 뽑아본 미국 반도체 섹터 리포트"
description: "Claude Code의 equity-research:sector-overview 스킬로 생성한 미국 반도체 섹터 분석입니다. 밸류체인, 경쟁 구도, 밸류에이션, 리스크 요인을 정리하고 모든 수치를 공시 원문과 대조했습니다."
date: 2026-07-26
update: 2026-07-26
series: "Claude 금융 스킬"
category: Stock
...
본문 내용은 Task 1의 원본 산출물에서 가져오되, Task 2 검증 표에서 `불일치`/`출처확인불가`로 판정된 항목은 제외한다. 목차는 아래 구조를 그대로 따른다.
...
`# 7`에는 Task 2 Step 6의 요약 숫자와 검증 표를 넣되, 표는 대표 사례 5행 이내로 줄인다. 검증 *방법*은 설명하지 말고 글 1로 링크한다(글 1이 아직 없으므로 링크 URL은 Task 7 완료 후 Task 8에서 채운다).

`# 9. 참고`에는 SEC EDGAR 링크, 각 사 IR 페이지, 사용한 웹 출처를 나열한다.

- [ ] **Step 4: 밸류체인 Mermaid 다이어그램을 넣는다**

`## 2.1` 안에 아래 형식으로 넣는다. 노드 라벨에 `%`, `~`, `/`, `(`, `)`가 들어가면 반드시 쌍따옴표로 감싼다.
