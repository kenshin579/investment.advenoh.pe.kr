...
| `1937-roosevelt-recession` | drawdown | peak 1937-02 / trough 1938-04 | 1937 불황 | 회복이 끝나기 전에 재정과 통화를 동시에 조인 결과, 시장이 다시 절반 가까이 빠졌다. |
| `1941-pearl-harbor-wwii` | drawdown | peak 1939-10 / trough 1942-04 | 2차 세계대전 | 개전 후 2년 반에 걸쳐 시장이 40% 가까이 빠졌고, 진주만 공습 넉 달 뒤 바닥을 쳤다. 그 뒤로는 전쟁 중에도 올랐다. |
...
| `1980-volcker-shock` | drawdown | peak 1980-11 / trough 1982-07 | 볼커 쇼크 | 물가를 잡기 위해 정책금리를 19% 위로 끌어올렸다. 두 번의 침체를 치르고 인플레이션이 꺾였다. |
| `1987-black-monday` | drawdown | peak 1987-08 / trough 1987-11 | 블랙먼데이 | 하루에 22.6% 빠졌다. 뚜렷한 촉발 사건 없이 프로그램 매매가 하락을 증폭시켰다. |
| `1997-asian-financial-crisis` | **moment** | at 1997-11 | IMF 외환위기 | 한국이 IMF 구제금융을 신청했다. 전후 1년 사이 한국 증시가 40% 넘게 빠지는 동안 S&P 500 은 오히려 50% 넘게 올랐다. |
...
**사건 날짜는 실제 데이터로 검증된 값이다.** 초안의 날짜 중 4개가 틀려서 바로잡았다
(2026-08-01, `data/timeline/series.json` 대조):

| 사건 | 초안 | 초안의 낙폭 | 수정 | 수정 후 |
|---|---|---:|---|---:|
| 1937 불황 | 1937-03~1938-03 | -43.01% | 1937-02~1938-04 | -45.39% |
| 2차 세계대전 | 1941-12~1942-04 | **-10.5%** | 1939-10~1942-04 | -39.22% |
| 볼커 쇼크 | 1980-02~1982-08 | **-4.86%** | 1980-11~1982-07 | -19.38% |
| IMF 외환위기 | 1997-07~1998-06 | **+19.79%** | `moment` at 1997-11 | S&P +55.6% / 한국 -42.1% |

IMF 외환위기는 미국 지수가 **오른** 사건이라 `drawdown` 으로 잡으면 위기 카드에 "+19.8%" 가 뜬다.
실제로는 한국이 무너지고 미국은 오른 사건이므로 `moment` 로 바꾸고, 그 괴리 자체를 요약문에 담았다.
한국 지표 배지가 -42.1% 를 보여주므로 카드가 이야기를 제대로 전달한다.

`moment` 의 ±12개월 창(1996-11~1998-11)은 한국 증시의 실제 고점→저점(-59.2%, 1997-06~1998-09)보다
낙폭이 작게 나온다. 창 끝이 이미 일부 회복된 시점이기 때문이다.
**요약문은 창이 실제로 보여주는 숫자에 맞춰 써야 한다** — 그러지 않으면 sp500 에서
`headlineDrawdown` 으로 막았던 것과 같은 모순(요약문 60% vs 배지 42%)이 다른 계열에서 생긴다.

`moment` 사건(`1971-nixon-shock`, `1997-asian-financial-crisis`)의 frontmatter는 `peak`/`trough` 대신 `at` 하나만 쓴다:
```yaml
event:
  kind: moment
  at: 1971-08
